...
@ccclass('UIBattleShipTicket')
export class UIBattleShipTicket extends Component {
    @property(Node)
    nodeIcon: Node = null;

    public static uistacks: UIBattleShipTicket[] = [];
    ...
    public static get current(): UIBattleShipTicket {
        try {
    ...
    protected onEnable(): void {
        ...
        ClientEvent.on(GameConstant.EVENT_NAME.USERBATTLESHIPTICKETS_UPDATED, this.updateData, this);
        UIBattleShipTicket.uistacks.push(this);
    }

    protected onDisable(): void {
        ClientEvent.off(GameConstant.EVENT_NAME.USERBATTLESHIPTICKETS_UPDATED, this.updateData, this);
        UIBattleShipTicket.uistacks = UIBattleShipTicket.uistacks.filter(ui => ui != this);
        this.tweenMain?.stop();
        this.tweenMain = null;
    }
    ...
    public updateData() {
        const checkValue = UserManager.instance.battleShipTickets ?? 0;
        this.tweenMain?.stop();
        this.tweenMain = tween(this.showValue)
            .to(0.35, { value: checkValue })
            .call(() => {
                this.tweenMain = null;
                this.labelMain.string = Utils.formatNumber(Math.round(this.showValue?.value));;
            })
            .start();
    }
    ...
    protected update(dt: number): void {
        ...
        const checkShown = Utils.formatNumber(Math.round(this.showValue?.value));
